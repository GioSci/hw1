function getAnni(){
    fetch("anni_accademici_get.php").then(onResponse).then(onJsonAnni);
}
function onJsonAnni(json){
    for(let element of json){
        const id=element.id;
        const anni=element.anni;
        const option=document.createElement('option');
        option.value=id;
        option.textContent=anni;
...
function search(event){
    ...
    const q=encodeURIComponent(keyWordIn.value)
...
const selectAnno=document.querySelector('#anno');
getAnni();
